...
export function drawViewportPinchRing(
  ...
  const proximity = Math.max(0, Math.min(1, 1 - distance / PINCH_FAR));
  const nearTrigger = proximity > 1 - PINCH_CLOSE / PINCH_FAR;
  const alpha = isGrasping
    ? 0.9
    : nearTrigger
      ? 0.55 + proximity * 0.35
      : 0.12 + proximity * 0.2;
  ...
  ctx.fillStyle = isGrasping ? accent.glow : accent.dim;
  ctx.lineWidth = isGrasping ? 2.5 : 1.5;
  const radius = 5 + proximity * 4;
  ctx.beginPath();
  ctx.arc(tip.x, tip.y, radius, 0, Math.PI * 2);
  if (isGrasping || nearTrigger) ctx.fill();
  ctx.stroke();
  ctx.restore();
}
