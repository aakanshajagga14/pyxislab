...
export interface ReportInput {
  ...
  trackingLostEvents?: TrackingLostEvent[];
...
export function buildLapSessionReport(input: ReportInput): LapSessionReport {
  ...
  return {
    ...
    trackingLostEvents: input.trackingLostEvents ?? [],
...
function generateRecommendations(
  ...
  const items: string[] = [];
  const trackingLostTotal = trackingLostEvents.reduce(
    (sum, e) => sum + e.durationSeconds,
    0
  );
  if (trackingLostEvents.length >= 3 || trackingLostTotal >= 15) {
    items.push(
      "Frequent tracking dropouts detected — improve front lighting, use a plain background, and keep both hands fully in frame at 40–70 cm from the camera"
    );
  }
